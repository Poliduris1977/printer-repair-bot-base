...
async def save_to_sheets(data: dict):
    try:
        ...
        row = [
            datetime.now().strftime('%Y-%m-%d %H:%M:%S'),               # 0: Дата
            data.get('username', 'Нет username'),                       # 1: Telegram Username
            data.get('company_name', 'Не указано'),                     # 2: Компания/Имя
            data.get('address', 'Не указан'),                           # 3: Адрес
            data.get('phone', 'Не указан'),                             # 4: Телефон
            data.get('printer_model', 'Не указана'),                    # 5: Модель
            data.get('issue', 'Не указана'),                            # 6: Проблема
            data.get('media_url', 'Нет фото/видео'),                    # 7: Фото/Видео URL
            data.get('desired_date', 'Не указана')                      # 8: Дата
        ]
...
async def on_shutdown(_):
    ...
    logger.info('Shutdown завершён: сессия закрыта, webhook удалён')
...
def main():
    ...
    async def shutdown_app():
        ...
        await asyncio.sleep(1)  # Время на завершение

    app.cleanup_ctx.append(lambda _: (yield, shutdown_app()))
